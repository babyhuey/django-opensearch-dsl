...
class DocumentTestCase(TestCase):
    ...
    def test_search_country(self):
        call_command(
            ...
            "index",
            "create",
            CountryDocument.Index.name,
            force=True,
            verbosity=0,
        )

        CountryDocument().update(
            CountryDocument().get_indexing_queryset(), "index", refresh=True
        )
        self.assertEqual(
            set(
                CountryDocument.search()
                .query("term", **{"continent.name": "Europe"})
                .extra(size=300)
                .to_queryset()
            ),
    ...
    def test_search_country_refresh_default_to_settings(self):
        call_command(
            ...
            "index",
            "create",
            ContinentDocument.Index.name,
            force=True,
            verbosity=0,
        )

        ContinentDocument().update(
            ContinentDocument().get_indexing_queryset(), "index", refresh=True
        )
    ...
    def test_update_instance(self):
        ...
        ContinentDocument().update(
            Continent.objects.get(countries__name="France"), "index", refresh=True
        )
        search = ContinentDocument.search().query(
            "nested", path="countries", query=Q("term", **{"countries.name": "France"})
        )
